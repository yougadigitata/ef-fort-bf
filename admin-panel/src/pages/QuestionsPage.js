...
export default function QuestionsPage({ onNavigate, onEdit }) {
    ...
    async function loadSeriesForMatiere(matiereCode) {
        if (!matiereCode) {
            setSeries([]);
            return;
        }
    ...
    async function handleDelete(id, soft = false) {
        if (!confirm(soft ? 'Masquer cette question ?' : 'Supprimer définitivement cette question ?'))
            return;
        try {
    ...
    return (<div>
      ...
      <div style={{
        ...
        <div style={{ flex: '2 1 220px' }}>
          ...
          <input type="text" value={filters.search} onChange={e => setFilters(f => ({ ...f, search: e.target.value }))} onKeyDown={handleSearchKeyDown} placeholder="Taper pour rechercher... (Entrée)" style={{ ...inputStyle, width: '100%' }}/>
        </div>
      ...
      <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', overflow: 'hidden' }}>
        {loading ? (<div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>⏳ Chargement...</div>) : questions.length === 0 ? (<div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#0f172a' }}>
                  {['#', 'Question', 'Matière', 'Série', 'Diff.', 'Rép.', 'Statut', 'Actions'].map(h => (<th key={h} style={{ padding: '10px 12px', color: '#64748b', fontSize: 12, fontWeight: 600, textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>{h}</th>))}
                </tr>
              </thead>
              <tbody>
                {questions.map((q, idx) => (<tr key={q.id} style={{
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      {q.published === false ? (<span style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444', padding: '2px 8px', borderRadius: 10, fontSize: 11 }}>Masqué</span>) : (<span style={{ background: 'rgba(74,222,128,0.1)', color: '#4ade80', padding: '2px 8px', borderRadius: 10, fontSize: 11 }}>Publié</span>)}
                    </td>
                    <td style={{ padding: '10px 12px' }}>
...
const selectStyle = { width: '100%', padding: '8px 10px', background: '#0f172a', border: '1px solid #334155', borderRadius: 6, color: '#e2e8f0', fontSize: 14 };
const inputStyle = { padding: '8px 10px', background: '#0f172a', border: '1px solid #334155', borderRadius: 6, color: '#e2e8f0', fontSize: 14 };
const paginationBtn = { padding: '6px 14px', background: '#334155', border: 'none', borderRadius: 6, color: '#e2e8f0', cursor: 'pointer', fontSize: 13 };
function MatiereChip({ matiereId, matieres }) {
...
function ActionBtn({ children, title, onClick, color = '#94a3b8' }) {
    return (<button onClick={onClick} title={title} style={{
            background: 'none', border: 'none', cursor: 'pointer', fontSize: 14,
            padding: '2px 4px', borderRadius: 4, transition: 'background 0.15s',
        }} onMouseEnter={e => (e.currentTarget.style.background = '#334155')} onMouseLeave={e => (e.currentTarget.style.background = 'none')}>{children}</button>);
}

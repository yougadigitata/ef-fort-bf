...
export default function BulkImportPage({ onNavigate }) {
    ...
    async function handleImport() {
        if (!validation?.questions)
            return;
        if (!confirm(`Importer ${validation.total_valid} questions ? Cette action est immédiate et visible par tous les utilisateurs.`))
            return;
        setLoading(true);
        setStep('import');
        try {
    ...
    async function handleCancelImport(importId) {
        if (!confirm('Annuler cet import et supprimer les questions importées ?'))
            return;
        try {
...
Qui dirige le DOGE ?;Trump;Musk & Ramaswamy;Faye;Sonko;B;DOGE créé en 2025...;MOYEN;[UUID_MATIERE]`}
  ...
  "matiere_id": "[UUID]"
      ...
      <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155', marginBottom: 20 }}>
        ...
        {file && (<button onClick={handleValidate} disabled={loading} style={{
      ...
      {validation && step === 'validate' && (<div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155', marginBottom: 20 }}>
          ...
          {/* Erreurs */}
          {validation.errors?.length > 0 && (<div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: 12, marginBottom: 16 }}>
              <div style={{ color: '#ef4444', fontWeight: 600, fontSize: 13, marginBottom: 8 }}>⚠️ Erreurs détectées :</div>
              {validation.errors.slice(0, 5).map((e, i) => (<div key={i} style={{ color: '#fca5a5', fontSize: 12, marginBottom: 4 }}>
          ...
          {validation.preview?.map((p, i) => (<div key={i} style={{ padding: '8px 12px', background: '#0f172a', borderRadius: 6, marginBottom: 6, fontSize: 12 }}>
          ...
          {validation.ready_to_import && (<button onClick={handleImport} disabled={loading} style={{
                    marginTop: 16, width: '100%', padding: 14,
      ...
      {/* En cours */}
      {step === 'import' && (<div style={{ background: '#1e293b', borderRadius: 12, padding: 40, border: '1px solid #334155', textAlign: 'center' }}>
      ...
      {/* Résultats */}
      {step === 'done' && importResult && (<div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid rgba(74,222,128,0.3)', marginBottom: 20 }}>
      ...
      {/* Historique */}
      {showHistory && (<div style={{ background: '#1e293b', borderRadius: 12, padding: 20, border: '1px solid #334155' }}>
          <h3 style={{ color: '#f1f5f9', fontSize: 15, fontWeight: 600, marginBottom: 16 }}>📜 Historique des imports</h3>
          {history.length === 0 ? (<p style={{ color: '#64748b', fontSize: 13 }}>Aucun import enregistré</p>) : history.map((imp) => (<div key={imp.id} style={{ padding: '10px 0', borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                {imp.status === 'success' && (<button onClick={() => handleCancelImport(imp.id)} title="Annuler cet import" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444', padding: '4px 8px', borderRadius: 6, cursor: 'pointer', fontSize: 12 }}>
                    🗑️ Annuler
                  </button>)}
              </div>
            </div>))}
...
function StatCard({ label, value, color }) {
    return (<div style={{ background: '#0f172a', borderRadius: 8, padding: 12, textAlign: 'center' }}>
...
const lbl = { display: 'block', color: '#94a3b8', fontSize: 12, marginBottom: 4, fontWeight: 500 };
const sel = { width: '100%', padding: '8px 10px', background: '#0f172a', border: '1px solid #334155', borderRadius: 6, color: '#e2e8f0', fontSize: 14 };

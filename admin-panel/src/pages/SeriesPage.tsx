...
import { getSeries, getMatieres, createSerie, updateSerie, deleteSerie, autoGenerateSerie, duplicateQuestion } from '../api';
...
export default function SeriesPage({ onNavigate }: { onNavigate: (page: Page) => void }) {
  ...
  const [editSerie, setEditSerie] = useState<any>(null);
  ...
  async function handleAutoGenerate() {
    if (!selectedMatiere) { showToast('⚠️ Sélectionner une matière d\'abord'); return; }
    if (!confirm('Créer une série automatique de 20 questions pour cette matière ?')) return;
    try {
  ...
  async function handleDelete(id: string) {
    if (!confirm('Supprimer cette série ? (les questions ne seront pas supprimées)')) return;
    try {
  ...
  return (
    <div>
      ...
      {/* Formulaire création */}
      {showCreate && (
        <div style={{ background: '#1e293b', borderRadius: 12, padding: 20, border: '1px solid #1A5C38', marginBottom: 20 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#0f172a' }}>
                {['N°', 'Titre', 'Questions', 'Niveau', 'Durée', 'Statut', 'Actions'].map(h => (

...
export default function SimulationsPage({ onNavigate }) {
    ...
    async function loadData() {
        ...
        try {
            ...
            setForm(f => ({
                ...f,
                config: (matData.matieres ?? []).map((m) => ({ matiere_id: m.id, matiere_nom: m.nom, count: 0 })),
            }));
        }
        catch (err) {
    ...
    async function handleCreate(e) {
        e.preventDefault();
        if (!form.titre.trim()) {
            showToast('⚠️ Titre requis');
            return;
        }
        if (totalSelected === 0) {
            showToast('⚠️ Sélectionner au moins 1 question');
            return;
        }
        setSaving(true);
        try {
    ...
    return (<div>
      ...
      {/* Formulaire création */}
      {showCreate && (<div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #1A5C38', marginBottom: 24 }}>
          ...
          <form onSubmit={handleCreate}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {DUREES.map(d => (<button key={d.value} type="button" onClick={() => setForm(f => ({ ...f, duree_minutes: d.value }))} style={{
            ...
            <div style={{ background: '#0f172a', borderRadius: 8, padding: 16, marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: 8 }}>
                {form.config.map((c, idx) => (<div key={c.matiere_id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: c.count > 0 ? 'rgba(26,92,56,0.2)' : '#1e293b', borderRadius: 6, border: c.count > 0 ? '1px solid rgba(26,92,56,0.5)' : '1px solid #334155' }}>
      ...
      {/* Liste simulations */}
      {loading ? (<div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>⏳ Chargement...</div>) : simulations.length === 0 ? (<div style={{ background: '#1e293b', borderRadius: 12, padding: 40, textAlign: 'center', border: '1px solid #334155' }}>
          ...
          {simulations.map((s) => (<div key={s.id} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  {s.published ? (<span style={{ background: 'rgba(74,222,128,0.1)', color: '#4ade80', padding: '2px 8px', borderRadius: 10, fontSize: 11 }}>LIVE</span>) : (<span style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444', padding: '2px 8px', borderRadius: 10, fontSize: 11 }}>Masquée</span>)}
                </div>
                ...
                <div style={{ display: 'flex', gap: 16, color: '#64748b', fontSize: 12 }}>
                  ...
                  <span>📚 {(() => { try {
                return JSON.parse(s.question_ids || '[]').length;
            }
            catch {
                return 0;
            } })()} questions</span>

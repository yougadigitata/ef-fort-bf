...
export default function FlagsPage({ onNavigate }) {
    ...
    return (<div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {statuses.map(s => (<button key={s.key} onClick={() => setStatus(s.key)} style={{
      ...
      {/* Dialog résolution */}
      {resolveId && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, maxWidth: 480, width: '90%', border: '1px solid #334155' }}>
            ...
            <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)} placeholder="Note admin (ex: Explication améliorée, réponse corrigée...)" rows={3} style={{ width: '100%', padding: '10px', background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#e2e8f0', fontSize: 14, resize: 'vertical', marginBottom: 16 }}/>
      ...
      {/* Liste */}
      {loading ? (<div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>⏳ Chargement...</div>) : flags.length === 0 ? (<div style={{ background: '#1e293b', borderRadius: 12, padding: 40, textAlign: 'center', border: '1px solid #334155' }}>
          ...
          {flags.map((f) => (<div key={f.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                <div style={{ flex: 1 }}>
                  ...
                  {/* Question préview */}
                  {f.questions?.enonce && (<div style={{ background: '#0f172a', borderRadius: 8, padding: '8px 12px', marginBottom: 10, fontSize: 13, color: '#e2e8f0' }}>
                  ...
                  {f.details && (<div style={{ color: '#64748b', fontSize: 13 }}>💬 {f.details}</div>)}
                  {f.admin_response && (<div style={{ background: 'rgba(26,92,56,0.1)', border: '1px solid rgba(26,92,56,0.3)', borderRadius: 8, padding: '6px 10px', marginTop: 8, color: '#4ade80', fontSize: 13 }}>
                      📝 Note admin: {f.admin_response}
                    </div>)}
                </div>

                {f.status !== 'resolved' && (<div style={{ display: 'flex', gap: 8, marginLeft: 16, flexShrink: 0 }}>

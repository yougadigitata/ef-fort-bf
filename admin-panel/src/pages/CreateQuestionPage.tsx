...
export default function CreateQuestionPage({ questionId, onNavigate }: Props) {
  ...
  useEffect(() => { loadMatieres(); if (isEdit && questionId) loadQuestion(questionId); }, []);
  ...
  async function loadSeriesForMatiere(matiereId: string) {
    if (!matiereId) { setSeries([]); return; }
  ...
  async function loadQuestion(id: string) {
    ...
    try {
      ...
      if (q.matiere_id) await loadSeriesForMatiere(q.matiere_id);
    } catch (err: any) {
  ...
  function validate(): string | null {
    if (!form.enonce.trim()) return 'L\'énoncé est requis.';
    if (!form.option_a.trim()) return 'L\'option A est requise.';
    if (!form.option_b.trim()) return 'L\'option B est requise.';
    if (!form.matiere_id) return 'La matière est requise.';
    if (!form.bonne_reponse) return 'La bonne réponse est requise.';
    return null;
  }
  ...
  if (loading) return <div style={{ color: '#64748b', padding: 40, textAlign: 'center' }}>⏳ Chargement...</div>;
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155', marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div>
              ...
              <select value={form.matiere_id}
                onChange={e => { setField('matiere_id', e.target.value); loadSeriesForMatiere(e.target.value); }}
                required style={sel}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea
              value={form.enonce} onChange={e => setField('enonce', e.target.value)}
          ...
          {(['A', 'B', 'C', 'D', 'E'] as const).map(letter => (
            <div key={letter} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <input
                ...
                onChange={e => setField(`option_${letter.toLowerCase()}`, e.target.value)}
                placeholder={`Proposition ${letter}${letter === 'A' || letter === 'B' ? ' (requise)' : ' (optionnelle)'}`}
                required={letter === 'A' || letter === 'B'}
                style={{ ...inp, flex: 1 }}
              />
              {form.bonne_reponse === letter && (
        ...
        <div style={{ background: '#1e293b', borderRadius: 12, padding: 24, border: '1px solid #334155', marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={form.explication} onChange={e => setField('explication', e.target.value)}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              ...
              <textarea value={form.pieges} onChange={e => setField('pieges', e.target.value)}
                placeholder="Erreurs fréquentes des candidats..."
                rows={2} style={{ ...inp, width: '100%', resize: 'vertical' }} />
            </div>
            <div>
              <label style={lbl}>Source</label>
              <input type="text" value={form.sources} onChange={e => setField('sources', e.target.value)}
                placeholder="ex: Journal officiel 2026..."
                style={{ ...inp, width: '100%' }} />
            </div>
          </div>
        </div>

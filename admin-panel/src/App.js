...
import { LayoutDashboard, FileQuestion, Upload, BookOpen, Target, Flag, History, LogOut, Menu, X } from 'lucide-react';
...
export default function App() {
    ...
    const navigate = (page, questionId) => {
        setCurrentPage(page);
        if (questionId)
    ...
    return (<AuthContext.Provider value={{ user, setUser, logout }}>
      <div style={{ display: 'flex', minHeight: '100vh', background: '#0f172a' }}>
        {/* Overlay mobile */}
        {sidebarOpen && window.innerWidth < 768 && (<div onClick={() => setSidebarOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 40 }}/>)}
...
function NavItem({ icon, label, active, onClick }) {
    return (<button onClick={onClick} style={{
        ...
        e.target.style.background = 'rgba(255,255,255,0.05)'; }} onMouseLeave={e => { if (!active)

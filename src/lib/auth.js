...
export async function hashPassword(password, sel) {
    const enc = new TextEncoder();
    const buf = await crypto.subtle.digest('SHA-256', enc.encode(password + sel + 'EfFort2026BF'));
...
export async function verifyJWT(token) {
    try {
        ...
        const sigBytes = Uint8Array.from(atob(sigPad + '='.repeat(padding)), c => c.charCodeAt(0));
        ...
        if (pl.exp < Math.floor(Date.now() / 1000))
            return null;
        return pl;
    }
    catch {

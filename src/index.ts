...
app.post('/api/statuts', async (c) => {
  ...
  const userIsAdmin = (payload as any).is_admin === true || profile?.is_admin === true;
...
app.put('/api/actualites/:id', async (c) => {
  ...
  if (body['titre']) updateData['titre'] = String(body['titre']).trim();
  if (body['contenu']) updateData['contenu'] = String(body['contenu']).trim();
  if (body['categorie']) updateData['categorie'] = body['categorie'];
  if ('actif' in body) updateData['actif'] = Boolean(body['actif']);
...
app.get('/api/examens/:id/questions', async (c) => {
  ...
  const parsedSerie = parseInt(serieParam, 10);
  ...
  // Série 1 (IDs 66-75), Série 2 (IDs 76-85), Série 3 (IDs 107-116)
  const EXAMEN_SIMULATION_MAP: Record<string, { serie1: number; serie2: number; serie3: number }> = {
    'exam_001': { serie1: 66, serie2: 76, serie3: 107 },  // Administration générale
    'exam_002': { serie1: 67, serie2: 77, serie3: 108 },  // Justice & sécurité
    'exam_003': { serie1: 68, serie2: 78, serie3: 109 },  // Économie & finances
    'exam_004': { serie1: 69, serie2: 79, serie3: 110 },  // Concours de la santé
    'exam_005': { serie1: 70, serie2: 80, serie3: 111 },  // Éducation & formation
    'exam_006': { serie1: 71, serie2: 81, serie3: 112 },  // Concours techniques
    'exam_007': { serie1: 72, serie2: 82, serie3: 113 },  // Agriculture & environnement
    'exam_008': { serie1: 73, serie2: 83, serie3: 114 },  // Informatique & numérique
    'exam_009': { serie1: 74, serie2: 84, serie3: 115 },  // Travaux publics & urbanisme
    'exam_010': { serie1: 75, serie2: 85, serie3: 116 },  // Statistiques & planification
  };

  const simId = EXAMEN_SIMULATION_MAP[examenId]
    ? (serieNum === 3 ? EXAMEN_SIMULATION_MAP[examenId].serie3
       : serieNum === 2 ? EXAMEN_SIMULATION_MAP[examenId].serie2
       : EXAMEN_SIMULATION_MAP[examenId].serie1)
    : undefined;
  ...
  const examenNom = serieNum === 3
    ? `${examenNomBase} — Série 3`
    : serieNum === 2
...
app.get('/api/simulations-admin', async (c) => {
  ...
  try {
    ...
    if (!error && data && data.length > 0) {
      ...
      const simulationsWithCount = data.map((sim: any) => {
        let totalQ = 0;
        try {
          let qIds: string[] = [];
          if (typeof sim.question_ids === 'string') {
            ...
            qIds = Object.values(sim.question_ids) as string[];
          }
...
app.post('/api/simulations-admin/:id/demarrer', async (c) => {
  ...
  if (simErr || !sim) return c.json({ error: 'Simulation introuvable ou non publiée.' }, 404);

  // Récupérer les IDs de questions
  let questionIds: string[] = [];
  try {
    questionIds = JSON.parse((sim as any).question_ids ?? '[]');
  } catch (_) {}

  if (questionIds.length === 0) return c.json({ error: 'Cette simulation ne contient aucune question.' }, 400);
  ...
  if ((sim as any).ordre_questions === 'random') {
  ...
  return c.json({
    ...
    simulation_titre: (sim as any).titre,
    duree_minutes: (sim as any).duree_minutes ?? 90,
    questions: questions.map(q => ({
      ...
      option_c: q.option_c,
      option_d: q.option_d,
      option_e: (q as any).option_e ?? null,
      difficulte: q.difficulte,
    })),
    total: questions.length,
    show_corrections: (sim as any).show_corrections !== false,
  });
});

...
import { cleanTel, makePasswordHash, verifyPassword, createJWT, verifyJWT } from '../lib/auth';
const auth = new Hono();
// Créer un user dans auth.users puis dans profiles
async function createUserWithProfile(env, data) {
    const db = getDB(env);
...
auth.post('/inscription', async (c) => {
    ...
    const { nom, prenom, telephone, niveau, password } = body;

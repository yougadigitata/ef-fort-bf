...
adminCms.post('/questions/bulk-import', requireAdmin, async (c) => {
    ...
    const seriesCacheByMatiere = {};
    async function getOrCreateSerieForBulk(qMatiereId) {
        if (!qMatiereId)
            return null;
...
function parseMdOrTxt(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const questionMatch = line.match(/^#{1,3}\s*(?:Question\s*\d*\s*:?\s*)?(.+)$/) ||
        ...
        if (optionMatch && currentQ) {
            ...
            const isBonne = optText.endsWith('*') ||
                optText.endsWith('✓') ||
                optText.endsWith('✔') ||
...
adminCms.post('/examens/bulk-import', requireAdmin, async (c) => {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (body.questions_text && typeof body.questions_text === 'string') {
                ...
                if (fmt === 'md' || fmt === 'txt') {
                    ...
                    try {
                        questions = JSON.parse(body.questions_text);
                    }
                    catch {
                        questions = parseCSV(body.questions_text);
                    }
                }
            }
            else {
    ...
    const duration = Math.round((Date.now() - startTime) / 1000);
    await logAdminAction(db, adminId, 'bulk_import_examen', 'simulation', simId ?? '0', null, { imported: importedCount, simulation_id: simId }, `Import examen: ${importedCount} questions`);
    return c.json({
        success: importedCount > 0,
        imported: importedCount,
        failed: failedCount,
...
adminCms.get('/examens', requireAdmin, async (c) => {
    ...
    const enriched = await Promise.all((data ?? []).map(async (sim) => {
...
adminCms.post('/exam-generator/preview', requireAdmin, async (c) => {
    ...
    for (const alloc of body.allocations) {
        ...
        if (questions) {
            ...
            allQuestions.push(...shuffled.map((q) => ({ ...q, matiere: matiereName })));
        }
    }
...
adminCms.post('/exam-generator/generate', requireAdmin, async (c) => {
    ...
    if (!examId || destination === 'simulation') {
        ...
        if (simErr)
            return c.json({ error: simErr.message }, 500);
        examId = newSim?.id;
    }
    if (!examId)
        return c.json({ error: 'Impossible de créer l\'examen.' }, 500);
    // Lier les questions à la simulation
    const liens = allQuestions.map((q, i) => ({
    ...
    if (lienErr) {
        ...
        if (lienErr2)
            return c.json({ error: `Erreur liaison questions: ${lienErr.message}` }, 500);
    }
    // Log audit
    try {
        await logAdminAction(db, adminId, 'generate_exam', 'simulation', examId, null, { titre, nb_questions: allQuestions.length, allocations }, `Examen composite généré: "${titre}" — ${allQuestions.length} questions`);
    }
    catch (_) { }
    return c.json({
        success: true,

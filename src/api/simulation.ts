...
async function requireAuth(c: any, next: any) {
  ...
  context.userId = payload['id'] as string;
  context.isAdmin = payload['is_admin'] as boolean;
  await next();
}
...
simulation.post('/demarrer', requireAuth, async (c) => {
  ...
  // Vérifier abonnement et quota freemium (table 'profiles' et non 'users')
  ...
  const byMatiere: Record<string, typeof available> = {};
  for (const q of available) {
    const mid = q.matiere_id ?? 'unknown';
    if (!byMatiere[mid]) byMatiere[mid] = [];
